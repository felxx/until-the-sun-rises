...
class GameManager:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    for idx in range(len(options)):
                        rect = pygame.Rect(self.width // 2 - 120, 220 + (idx * 50), 240, 40)
    ...
    def draw_menu(self):
        ...
        self.screen.blit(title_surf, title_surf.get_rect(center=(self.width // 2, 110)))

        options = self.menu_options
        start_y = 220 if len(options) > 4 else 240

        for idx, option in enumerate(options):
            is_selected = (idx == self.selected_index)
            color = (255, 255, 255) if is_selected else (160, 160, 160)
            
            rect = pygame.Rect(self.width // 2 - 120, start_y + (idx * 50), 240, 40)
            if is_selected:
