...
class GameWorld:
    def __init__(self):
        ...
        # Parâmetros de tempo e transição de claridade
        self.DAWN_DURATION = (180.0 * 3)  # Tempo em segundos para a escuridão virar dia totalmente 
        self.NIGHT_COLOR = (10, 10, 20)  # Cor do breu total
        self.DAY_COLOR = (255, 255, 255) # Iluminação total (dia)

        # Parâmetros da Lanterna / Luz
        self.flashlight_range = 280  # Alcance da lanterna em pixels
        self.flashlight_angle = 50   # Ângulo de abertura do cone em graus
        self.aura_radius = 300        # Raio da luz em volta do jogador
        
        # Pré-gera a textura de luz suave
    ...
    def _create_light_texture(self):
        """Cria uma superfície com gradiente suave contendo a aura radial e o cone da lanterna."""
        size = self.flashlight_range * 2 + 100
        center = (size // 2, size // 2)
        surface = pygame.Surface((size, size), pygame.SRCALPHA)

        # 1. Aura Radial do jogador
        aura_steps = 50
        for i in range(aura_steps, 0, -1):
            r = int(self.aura_radius * (i / aura_steps))
            factor = 1.0 - (i / aura_steps)
            intensity = int(200 * (factor ** 2.5)) # Decaimento suave
        ...
        for i in range(cone_steps, 0, -1):
            ...
            for seg in range(num_segments + 1):
                angle_rad = math.radians(-half_angle + (self.flashlight_angle * seg / num_segments))
                px = center[0] + curr_dist * math.cos(angle_rad)
                py = center[1] + curr_dist * math.sin(angle_rad)
    ...
    def _draw_fog(self, screen):
        """Calcula o nível atual de escuridão e desenha a lanterna e aura sobre o jogo."""
        # 1. Calcula a transição progressiva da cor ambiente de acordo com o tempo decorrido
